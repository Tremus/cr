...
// To save states automatically from previous instance to a new loaded one, use
// CR_STATE flag on statics/globals. This will create a new data section in the
// binary for transferable states between instances that will be copied and kept
// around to overwrite the new instance initial states. That means that things
// that were initialized in a previous loaded states will continue from where
// they were.
static unsigned int CR_STATE version = 1;
// But unfortunately new added states in new instances (post the fact states) do
// not work because they will be overwritten by previous states (in this case,
// inexisting ones being zeroed), so uncommenting the following line will have a
// value of zero.
// static int32_t CR_STATE sad_state = 2;
// For this reason, cr is better for runtime modifing existing stuff than
// adding/removing new stuff. At the sime time, if we remove a state variable,
// the reload will safely fail with the error CR_STATE_INVALIDATED. A rollback
// will be effectued and this error can be dealt client side, for exemple, by
...
CR_EXPORT int cr_main(struct cr_plugin *ctx, enum cr_op operation) {
    ...
    // each time we load an instance this value will be reset to its initial
